"use client"
import { codeModify, getAllTables, getTableDetail, getTableFieldsById } from "@/service/code-gen"
import { useAllDictData } from "@/service/dict-datum"
import type { Field, TableResult } from "@/types/code-gen"
import { Button, Checkbox, Form, Input, Modal, Select, Table, Tabs, type TabsProps, message } from "antd"
...
const CodeModify: React.FC<CodeEditProps> = ({ open, onClose, tableId }) => {
  ...
  useEffect(() => {
    if (open && tableId) {
      ; (async () => {
        ...
        if (table.options) {
          try {
            parsedOptions = JSON.parse(table.options)
            if (table.tpl_category === "2" || table.tpl_category === "4") {
              tableForm.setFieldsValue({
                ...table,
                relationTables: { ...parsedOptions },
              })
            } else if (table.tpl_category === "3") {
              tableForm.setFieldsValue({
                ...table,
                treeTable: { ...parsedOptions },
              })
            }

          } catch (error) {
            console.error("Failed to parse table options:", error)
          }
        }
  ...
  useEffect(() => {
    if ((watchTplCategory === "2" || watchTplCategory === "4") && Array.isArray(watchSubTables)) {
  ...
  const fieldColumns = [
    { title: "主键", dataIndex: "id", key: "id", hidden: true },
    {
      ...
      dataIndex: "No",
      key: "No",
      render: (_: any, _r: Field, i: number) => i + 1,
      width: 60,
    },
    ...
    },
    {
      ...
      render: (v: string, r: Field) => {
        const options =
          dictData && typeof dictData === "object"
            ? Object.entries(dictData as Record<string, Array<{ label: string }>>).map(([k, arr]) => ({
              value: k,
              label: Array.isArray(arr) ? arr.map((i) => i.label).join(",") : "",
            }))
            : []
        return (
          <Select
    ...
    },
    {
      ...
      children: (
        <Form layout="horizontal" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} form={tableForm}>
          ...
          {(watchTplCategory === "2" || watchTplCategory === "4") && (
              ...
              {(fields, { add, remove }) => (
                <>
                  ...
                  {fields.map((field, idx) => {
                    ...
                    return (
                      <div key={field.key} className="flex items-center gap-4 mb-3 px-4 h-12">
                        <Form.Item
                          label="关联表名"
                        ...
                        <Form.Item
                          ...
                          style={{ flex: 1, marginBottom: 0 }}
                        >
                          <Select placeholder="请选择字段" options={relationOptions} />
                        </Form.Item>
                        <Form.Item
                          label="显示字段"
                          name={[field.name, "relationFieldName"]}
                          rules={[{ required: true, message: "请选择显示字段" }]}
                          style={{ flex: 1, marginBottom: 0 }}
                        >
                          <Select placeholder="请选择显示字段" options={relationOptions} />
                        </Form.Item>
                        <div className="flex gap-2 mt-1 min-w-26">
                          {fields.length > 1 && (
                            <Button size="small" className="btn-remove border-none cursor-pointer" onClick={() => remove(field.name)}>
                              删除
                            </Button>
                          )}
                          {idx === fields.length - 1 && (
                            <Button className="border-none btn-operation cursor-pointer" size="small" onClick={() => add()}>
                              新增
                            </Button>
                          )}
                        </div>
          ...
          {watchTplCategory === "3" && (
            <>
              <h4 className="mt-4 mb-4 font-semibold border-b border-gray-200">树表配置</h4>
