...
const CodeModify: React.FC<CodeEditProps> = ({ open, onClose, tableId }) => {
  ...
  const fieldColumns = [
    ...
    },
    {
      ...
      render: (v: string, r: Field) => (
        ...
        >
          {['String', 'Number', 'Boolean', 'Date', 'Array', 'Object'].map(
            (t) => (
  ...
  const handleCodeModifyFinish = async () => {
    const updatedFieldInfo = fieldInfo.map((f) =>
      changedFields[f.id] ? { ...f, ...changedFields[f.id] } : f,
    );
        ...
        >
          ...
          {watchTplCategory === '2' && (
            <Form.List name="relationTables" initialValue={[{}]}>
              {(fields, { add, remove }) => (
                <>
                  <h4 className="mt-4 mb-4 font-semibold border-b border-gray-200 px-4">
                    关联表配置
                  </h4>
                  {fields.map((field, idx) => {
                    const selectedTableName = tableForm.getFieldValue([
                      'relationTables',
                      field.name,
                      'tableId',
                    ]);
                    const relationOptions = (
                      subTableFields[selectedTableName] || []
                    ).map((f) => ({
                      value: f.field_name,
                    ...
                    return (
                      <div
                        key={field.key}
                        className="flex items-center gap-4 mb-3 px-4"
                      >
                        <Form.Item
                          label="关联表名"
                          ...
                          <Select
                            ...
                            onChange={() => {
                              tableForm.setFieldValue(
                                ['relationTables', field.name, 'relationField'],
                                undefined,
                              );
                            }}
                          />
