...
const DictType: React.FC = () => {
  ...
  const [dictTypeDetail, setDictTypeDetail] = useState<DictTypeDetail | null>(
    null,
  );
  ...
  const dictTypePageColumns: ColumnsType<DictTypePage> = [
    ...
    },
    {
      ...
      dataIndex: 'No',
      key: 'No',
      render: (_: number, _record: DictTypePage, rowIndex: number) =>
        rowIndex + 1,
      width: '8%',
    },
    {
        ...
        if (text) {
          return (
            <a
              ...
              href={`/system/dict-data?type=${text}`}
            >
              {text}
            </a>
          );
  ...
  const [visibleColumns, setVisibleColumns] = useState(
    dictTypePageColumns.map((col) => col.key),
  );
  ...
  const filteredDictTypeColumns = dictTypePageColumns.filter((col) =>
  ...
  const onDictTypeQueryFinish = async () => {
    ...
    if (create_time) {
      ...
      dictTypeQueryFormData.create_time = [
    ...
    const filteredDictTypeQuery = Object.fromEntries(
      Object.entries(dictTypeQuery).filter(
  ...
  // 新增模块
  const [isDictTypeCreateModalVisible, setIsDictTypeCreateModalVisible] =
    useState(false);
  ...
  // 单个更新模块
  const [isDictTypeModifyModalVisible, setIsDictTypeModifyModalVisible] =
    useState<boolean>(false);
  const [isDictTypeModifyLoading, setIsDictTypeModifyLoading] =
    useState<boolean>(false);
  ...
  const handleDictTypeModifyFinish = async () => {
    const dictTypeModifyData =
      (await dictTypeModifyForm.validateFields()) as DictTypeModify;
  ...
  const [
    ...
    setIsDictTypeBatchModifyModalVisible,
  ] = useState<boolean>(false);
  const [isDictTypeBatchModifyLoading, setIsDictTypeBatchModifyLoading] =
    useState<boolean>(false);
  ...
  const handleDictTypeBatchModifyFinish = async () => {
    const dictTypeBatchModify =
      (await dictTypeBatchModifyForm.validateFields()) as DictTypeBatchModify;
  ...
  // 导入模块
  const [isDictTypeImportModalVisible, setIsDictTypeImportModalVisible] =
    useState<boolean>(false);
  const [isDictTypeImportLoading, setIsDictTypeImportLoading] =
    useState<boolean>(false);
