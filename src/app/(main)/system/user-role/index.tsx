...
const UserRole: React.FC = () => {
  ...
  const [userRolePageDataSource, setUserRolePageDataSource] = useState<
    UserRolePage[]
  >([]);
  ...
  const [userRoleDetail, setUserRoleDetail] = useState<UserRoleDetail | null>(
    null,
  );
  ...
  const userRolePageColumns: ColumnsType<UserRolePage> = [
    ...
    },
    {
      ...
      dataIndex: 'No',
      key: 'No',
      render: (_: number, _record: UserRolePage, rowIndex: number) =>
        rowIndex + 1,
      width: '8%',
    },
    {
  ...
  const [visibleColumns, setVisibleColumns] = useState(
    userRolePageColumns.map((col) => col.key),
  );
  ...
  const filteredUserRoleColumns = userRolePageColumns.filter((col) =>
  ...
  const onUserRoleQueryFinish = async () => {
    ...
    if (create_time) {
      ...
      userRoleQueryFormData.create_time = [
    ...
    const filteredUserRoleQuery = Object.fromEntries(
      Object.entries(userRoleQuery).filter(
  ...
  // 新增模块
  const [isUserRoleCreateModalVisible, setIsUserRoleCreateModalVisible] =
    useState(false);
  ...
  // 单个更新模块
  const [isUserRoleModifyModalVisible, setIsUserRoleModifyModalVisible] =
    useState<boolean>(false);
  const [isUserRoleModifyLoading, setIsUserRoleModifyLoading] =
    useState<boolean>(false);
  ...
  const handleUserRoleModifyFinish = async () => {
    const userRoleModifyData =
      (await userRoleModifyForm.validateFields()) as UserRoleModify;
  ...
  const [
    ...
    setIsUserRoleBatchModifyModalVisible,
  ] = useState<boolean>(false);
  const [isUserRoleBatchModifyLoading, setIsUserRoleBatchModifyLoading] =
    useState<boolean>(false);
  ...
  const handleUserRoleBatchModifyFinish = async () => {
    const userRoleBatchModify =
      (await userRoleBatchModifyForm.validateFields()) as UserRoleBatchModify;
  ...
  // 导入模块
  const [isUserRoleImportModalVisible, setIsUserRoleImportModalVisible] =
    useState<boolean>(false);
  const [isUserRoleImportLoading, setIsUserRoleImportLoading] =
    useState<boolean>(false);
  const [userRoleCreateList, setUserRoleCreateList] = useState<
    UserRoleCreate[]
  >([]);
